...
Unknown or omitted fields fail closed except the two optional 2.4 budget fields, which may be present with value `true` or absent:

- `wallClockApproved: true` — permits a worker wall clock above the default **45 minutes**. A manifest requesting more than 45 minutes **without** this flag is **rejected** (`manifest_timeout_not_approved`); it is never silently clamped. Longer runs are an explicit decision, never the default.
- `highEffortApproved: true` — permits a route `variant` or an `opencode.config` effort value of `high`, `xhigh`, or the selected provider's equivalent high-cost value (`max`). The default effective effort is `medium`; anything costlier requires this explicit field. Route values are validated against the selected provider's supported effort set or an explicitly declared model variant; unknown values are rejected as `manifest_effort_unknown` even when approved.

Both flags are recorded in the run summary next to the selected route for the independent audit. `commit.allowed` grants only the deterministic launcher permission to seal the exact candidate. Validation commands are direct executable/argument arrays with checkout-contained working directories and no shell tokenization, operators, interpolation, redirects, or pipelines. Write the manifest once, compute and record the lowercase SHA-256 over its exact bytes, and classify every declared external artifact before launch.
