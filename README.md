...
An approved plan may route a task to an external runtime only when it names the runtime, agent, model, variant, target directory, file contract, worker exploration permissions, mandatory validation argv, timeout, policy, launcher candidate contract, and session/artifact cleanup policy. The inherited Claude orchestrator writes one secret-free, hashed `airlock.external-agent/v2` manifest and invokes the launcher directly, exactly once, in the foreground:

```text
node "${CLAUDE_PLUGIN_ROOT}/scripts/run-external-agent.mjs" --manifest <absolute-json-path> --sha256 <lowercase-hex>
```

Airlock 1.3.1 implements this plan-approved multimodel [OpenCode worker](adapters/opencode/README.md) path. It requires Node.js, Claude Code, a direct Git executable, and the OpenCode CLI with the selected provider/model available; on Windows the launcher requires direct `opencode.exe` and `git.exe` paths, not PowerShell or npm command shims. `external-runner` is retained only as superseded compatibility documentation and performs no dispatch.

The worker owns only scoped investigation, edits, and approved exploratory checks. It has no Git write or candidate-claim authority. The launcher owns baseline/delta checks, mandatory final validation, exact staging, one candidate commit, post-commit audit, process handling, and cleanup; the orchestrator independently audits the launcher-sealed candidate and alone owns process artifacts, the separate ledger Crossing, push, and publication. External writers are serialized per checkout, and the orchestrator remains idle in that checkout while the launcher runs.

Mandatory validations use direct executable and argument arrays with `shell: false`, checkout-contained working directories, bounded output, timeouts, expected exits, and no validation-created delta. Before sealing, the launcher rejects custom Git filters on owned paths, uses a verified empty task-owned hooks directory, disables signing, stages only exact paths, audits cached names and cached `diff --check`, and verifies parent, count, message bytes, tree, paths, index, and status. It never resets or rewrites history.

Each bounded launcher result records its session, classification, route, validation and sealing evidence, candidate, and exact artifact state. For a missing summary, inspect only after proving the launcher tree is quiescent: unchanged `HEAD` with confined owned edits is the no-commit/no-candidate state; one exact child commit is the one-commit state and requires independent audit; every other state is indeterminate. A cleanup failure after commit preserves that commit and blocks acceptance. The launcher deletes only exact declared temporary sessions, manifest, evidence, message, hooks, and runtime paths; unknown state is left untouched.
